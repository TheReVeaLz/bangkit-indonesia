            ...
            <table class="table table-bordered mt-3">
                <thead>
                    <tr>
                        ...
                        <th style="width:90px;" scope="col">Action</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $i = 1;
                    $data = mysqli_query($con, "SELECT * FROM artikel inner join kategori on artikel.id_kategori=kategori.id_kategori ORDER BY id_artikel DESC");
                    foreach ($data as $row) :  ?>
                        <tr>
                            <th scope="row"><?= $i++ ?></th>
                            <td><?= $row['judul'] ?></td>
                            ...
                            <td style="line-height: 2.5;">
                                <a class="badge badge-success" style="padding:10px;width:100%;" href="artikel_edit.php?id_artikel=<?= $row['id_artikel'] ?>">Edit</a><br>
                                <a class="badge badge-danger" style="padding:10px;width:100%;" href="artikel_delete.php?id_artikel=<?= $row['id_artikel'] ?>" onclick="return confirm('Yakin ingin menghapus data ini?')">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach;
                    ?>
